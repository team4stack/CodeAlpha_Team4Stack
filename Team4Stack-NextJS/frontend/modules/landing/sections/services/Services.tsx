      ...
      {selectedService && (
        <>
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
              <div 
                className="bg-gray-900 rounded-2xl border border-white/10 shadow-2xl max-w-md w-full p-6 relative text-white"
                onClick={(e) => e.stopPropagation()}
              >
                <button
                  onClick={() => setSelectedService(null)}
                  className="services-modal-close-btn flex items-center justify-center rounded-full transition-colors"
                  aria-label="Close"
                >
