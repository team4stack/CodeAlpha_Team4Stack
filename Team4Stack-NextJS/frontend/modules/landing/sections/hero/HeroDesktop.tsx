...
const HeroDesktop: React.FC<HeroDesktopProps> = ({
  ...
  // Re-trigger grid animation whenever hero section re-enters viewport
  useEffect(() => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && entry.intersectionRatio > 0.4) {
            // Increment key so CSS animation restarts cleanly
  ...
  return (
    <section
      id="home"
      ref={sectionRef}
      className="relative overflow-hidden flex items-center justify-center min-h-[88vh] md:min-h-[92vh] lg:min-h-screen pt-20 md:pt-24"
    >
      {/* Structured Data */}
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="pointer-events-none absolute inset-0 z-[1]" aria-hidden="true">
          {/* Large grid with subtle pulse animation (restarts when hero comes back into view) */}
          <div
            key={gridCycle}
            className="hero-grid-animate absolute left-1/2 top-1/2 w-[170vmax] h-[170vmax] rounded-[48px] opacity-35"
            style={{
            animation: prefersReducedMotionState ? 'none' : 'heroGridPulse 8s ease-in-out infinite',
      ...
      <div className="container-custom relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-10 lg:gap-14 mb-8">
          {/* Left: Content */}
