...
const AdmissionForm: React.FC = () => {
  ...
  const buildAdmissionPayload = (isFinalSubmit: boolean) => {
  ...
  const persistDraftToDatabase = async (isFinalSubmit: boolean): Promise<boolean> => {
    if (!isAuthenticated) {
      showSignInPopup()
      return false
    }

    const payload = buildAdmissionPayload(isFinalSubmit)
  ...
  const onFinalSubmit: SubmitHandler<AdmissionFormValues> = async () => {
    ...
    try {
      ...
      const saved = await persistDraftToDatabase(true)
      if (!saved) return

      const payload = buildAdmissionPayload(true)
      const lines = [
        `Name: ${payload.name}`,
      ...
      const waURL = getWhatsAppUrl(
        phoneNumber,
        `New Course Inquiry\nName: ${payload.name}\nFather Name: ${payload.father_name}\nPhone: ${payload.phone}\nEmail: ${payload.email}\nCourse: ${payload.course_name}${uploadedScreenshotUrl ? `\nPayment Screenshot: ${uploadedScreenshotUrl}` : ''}`
      )
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0f1f] via-[#0b1226] to-[#060b18] pt-20 md:pt-28 pb-12 px-4 relative overflow-hidden">
      ...
      <div className="container-custom max-w-7xl mx-auto relative z-10 px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 md:mb-12">
          ...
          <div className="mt-4 max-w-3xl mx-auto rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                  {STEP_META.map((item) => {
                    ...
                    return (
                      <div
                        key={item.step}
                        className="rounded-xl px-3 py-2.5 sm:px-4 transition-all duration-300 bg-transparent"
                      >
                        <div className="flex items-center justify-center sm:justify-start gap-2.5">
                          ...
                          <div className="text-center sm:text-left">
                            <p className={`text-sm font-semibold ${isActive ? 'text-white' : 'text-white/70'}`}>{item.title}</p>
            ...
            <form onSubmit={handleSubmit(onFinalSubmit)} className="space-y-6">
              {currentStep === 1 && (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div>
                      <label htmlFor="name" className="block text-sm font-semibold text-white/90 mb-2.5">Full Name *</label>
                    ...
                    <div>
                      <label htmlFor="fatherName" className="block text-sm font-semibold text-white/90 mb-2.5">Father Name *</label>
                    ...
                    <div>
                      <label htmlFor="phone" className="block text-sm font-semibold text-white/90 mb-2.5">Phone Number *</label>
                    ...
                    <div>
                      <label htmlFor="email" className="block text-sm font-semibold text-white/90 mb-2.5">Email Address *</label>
                    ...
                    <div>
                      <label htmlFor="cnic" className="block text-sm font-semibold text-white/90 mb-2.5">CNIC / B-Form *</label>
                    ...
                    <div>
                      <label htmlFor="gender" className="block text-sm font-semibold text-white/90 mb-2.5">Gender *</label>
                    ...
                    <div>
                      <label htmlFor="dateOfBirth" className="block text-sm font-semibold text-white/90 mb-2.5">Date of Birth *</label>
                    ...
                    <div>
                      <label htmlFor="courseName" className="block text-sm font-semibold text-white/90 mb-2.5">Primary Course *</label>
                      {loadingCourses ? (
              ...
              {currentStep === 2 && (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div>
                      <label htmlFor="address" className="block text-sm font-semibold text-white/90 mb-2.5">Address</label>
                    ...
                    <div>
                      <label htmlFor="courseName2" className="block text-sm font-semibold text-white/90 mb-2.5">Second Course (Optional)</label>
                      {loadingCourses ? (
                        <div className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white/70">Loading courses...</div>
                      ) : (
              ...
              {currentStep === 3 && (
                <div className="space-y-6">
                  <div>
                    <label htmlFor="paymentScreenshot" className="block text-sm font-semibold text-white/90 mb-2.5">Payment Screenshot *</label>
                    <input
                      id="paymentScreenshot"
                      type="file"
                      accept="image/jpeg,image/jpg,image/png,image/webp"
                      onChange={handleFileChange}
                      className={inputClass(Boolean(fileError))}
                    />
                    {fileError && <p className="mt-2 text-xs sm:text-sm text-red-400">{fileError}</p>}
                    <p className="mt-2 text-xs text-gray-400">Accepted: JPG, PNG, WebP (max 2MB)</p>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <label htmlFor="termsCheckbox" className="text-sm text-white/90">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                {currentStep > 1 && (
                  <button
                    ...
                    className="sm:w-auto w-full px-5 py-3 rounded-xl border border-white/20 text-white hover:bg-white/10 transition-colors"
                ...
                {currentStep < 3 ? (
                  <button
                    ...
                    className="sm:ms-auto sm:w-auto w-full px-6 py-3 rounded-xl bg-linear-to-r from-cyan-500 via-purple-500 to-pink-500 text-white font-semibold hover:from-cyan-600 hover:via-purple-600 hover:to-pink-600 transition-all shadow-lg shadow-cyan-900/20"
                    disabled={isSubmitting || isSavingStep}
                  >
                  ...
                  <button
                    type="submit"
                    className="sm:ms-auto sm:w-auto w-full px-6 py-3 rounded-xl bg-linear-to-r from-cyan-500 via-purple-500 to-pink-500 text-white font-semibold hover:from-cyan-600 hover:via-purple-600 hover:to-pink-600 transition-all disabled:opacity-50 shadow-lg shadow-cyan-900/20"
                    disabled={isSubmitting || isSavingStep || !acceptedTerms}
                  >
