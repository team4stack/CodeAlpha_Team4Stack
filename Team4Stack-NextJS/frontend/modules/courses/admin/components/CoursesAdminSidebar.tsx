...
const CoursesAdminSidebar: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <aside
        className={`${isCollapsed ? 'w-20' : 'w-64'} flex h-full flex-col relative bg-black/40 backdrop-blur-2xl border-r border-cyan-500/20 shadow-2xl shadow-cyan-500/10 transition-[width] duration-300 ease-in-out overflow-visible`}
      >
        ...
        {/* Top: minimal strip for pin only – no extra empty space */}
        ...
        {/* Middle: nav only. No scrollbar added for pin – pin visibility is fixed by layout (content margin + z-index). */}
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-visible relative z-10">
          <nav className="p-4 pt-2">
            <ul className="space-y-2">
              {links.map((link, index) => {
                ...
                const isDashboard = link.to === '/admincourset4s'
                return (
