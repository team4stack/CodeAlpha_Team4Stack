      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-8 right-8 z-[60] w-[min(calc(100vw-4rem),392px)] flex flex-col overflow-hidden rounded-2xl bg-white/95 dark:bg-slate-800/95 shadow-2xl ring-1 ring-black/5 dark:ring-white/10 backdrop-blur-xl"
          ...
          >
            ...
            {/* Messages area */}
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-4 min-h-[220px] chat-panel-scroll">
                    ...
                    <div
                      className={`group relative max-w-[88%] rounded-2xl px-4 py-2.5 text-[15px] leading-snug ${
                        m.role === 'user'
                          ...
                          : 'rounded-bl-md bg-slate-100 dark:bg-slate-700/90 text-slate-800 dark:text-slate-100 shadow-sm border border-slate-200/60 dark:border-slate-600/50'
                      }`}
                    >
                      <p className="whitespace-pre-wrap">{m.text}</p>
                      <span className={`mt-1.5 block text-[11px] ${m.role === 'user' ? 'text-white/70' : 'text-slate-500 dark:text-slate-400'}`}>
                        {m.time}
                      </span>
                    </div>
                  </div>
            ...
            {/* Input area */}
            <div className="shrink-0 border-t border-slate-200/80 dark:border-slate-600/50 bg-slate-50/80 dark:bg-slate-800/50 px-4 py-3 rounded-b-2xl">
              <div className="flex gap-2.5">
                <input
                  ...
                  className="flex-1 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/80 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-400 transition-shadow"
                />
                <button
                  onClick={sendMessage}
                  disabled={!input.trim()}
                  className="rounded-xl bg-gradient-to-br from-violet-600 to-purple-600 px-4 py-3 text-white shadow-lg disabled:opacity-40 disabled:cursor-not-allowed transition-opacity focus:outline-none focus:ring-2 focus:ring-violet-400 focus:ring-offset-2 focus:ring-offset-slate-50 dark:focus:ring-offset-slate-800 active:scale-[0.98]"
        ...
        {!isOpen && (
          ...
          >
            <button
              ...
              className="bg-violet-600 hover:bg-violet-700 text-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 animate-bounce-slow w-14 h-14 flex items-center justify-center overflow-clip focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2"
