...
const ContentPage: React.FC = () => {
          ...
          : (isContact || isFooter || isHero)
              ? supabase
                  ...
                  .in('key', [
                    ...
                    ...(isFooter ? ['footer_about_text','footer_socials_json','footer_version'] : []),
  ...
  // Reset footer socials UI when switching away/back to footer
  useEffect(() => {
    if (!isFooter) {
    ...
    if (isFooter) {
      ...
      if (entries.length) {
        ...
        const { data: refreshed } = await supabase
          .from('site_settings')
          .select('key,value')
          .in('key', ['footer_about_text','footer_socials_json','footer_version'])
        setRows(refreshed as any || [])
