...
const AdminHeader: React.FC = () => {
  ...
  const prettyTitle = () => {
    ...
    if (p.startsWith('/adminlandingt4s')) {
      p = p.replace('/adminlandingt4s', '') || '/'
    } else if (p.startsWith('/adminstackt4s')) {
      p = p.replace('/adminstackt4s', '') || '/'
    } else if (p.startsWith('/adminteamt4s')) {
      p = p.replace('/adminteamt4s', '') || '/'
    } else if (p.startsWith('/admincourset4s')) {
      p = p.replace('/admincourset4s', '') || '/'
    } else if (p.startsWith('/supadmin')) {
      p = p.replace('/supadmin', '') || '/'
    }
    
    if (p === '/' || p === '') return 'Dashboard'
    const seg = p.split('/')[1]
    const map: Record<string,string> = { 
      hero: 'Hero Section', projects: 'Projects', services: 'Services', reviews: 'Reviews', 
      courses: 'Courses', team_members: 'Team', mentor_profile: 'Mentor', contact: 'Contact', 
      footer: 'Footer', forms: 'Admission Form', support: 'Support', settings: 'Settings',
      products: 'Products', categories: 'Categories', orders: 'Orders', sellers: 'Sellers',
      members: 'Team Members', mentor: 'Mentor Profile', roles: 'Roles & Positions'
    }
    return map[seg] || seg
  }
